...
in
{
  ...
  testFieldIcmpv6PacketTooBig = pe dsl.fields.icmpv6.packetTooBig {
    payload = { protocol = "icmpv6"; field = "packet-too-big"; };
  };
  testFieldEtherSaddr = pe dsl.fields.ether.saddr {
    payload = { protocol = "ether"; field = "saddr"; };
  };
  testFieldVlanId = pe dsl.fields.vlan.id {
    payload = { protocol = "vlan"; field = "id"; };
  };

  # -- Field trees — meta / ct / rt / socket / fib / ipsec / tunnelMeta -----

  testFieldMetaMark = pe dsl.fields.meta.mark { meta = { key = "mark"; }; };
  testFieldMetaIifname = pe dsl.fields.meta.iifname { meta = { key = "iifname"; }; };
  ...
  testPayloadEscape = pe (dsl.payload { protocol = "tcp"; field = "dport"; }) {
    payload = { protocol = "tcp"; field = "dport"; };
  };

  testPayloadRaw = pe (dsl.payloadRaw { base = "nh"; offset = 72; len = 16; }) {
    payload = { base = "nh"; offset = 72; len = 16; };
  };

  testPayloadTunnel = pe (dsl.payloadTunnel {
    tunnel = "vxlan";
    protocol = "ip";
    field = "daddr";
  ...
  testTcpOptionRaw = pe (dsl.expr.tcpOptionRaw { base = 2; offset = 16; len = 16; }) {
    "tcp option" = { base = 2; offset = 16; len = 16; };
  };

  testIpOption = pe (dsl.expr.ipOption { name = "lsrr"; field = "length"; }) {
    "ip option" = { name = "lsrr"; field = "length"; };
  };

  testSctpChunk = pe (dsl.expr.sctpChunk { name = "data"; field = "tsn"; }) {
    "sctp chunk" = { name = "data"; field = "tsn"; };
  };

  testDccpOption = pe (dsl.expr.dccpOption 42) { "dccp option" = { type = 42; }; };

  testExthdr = pe (dsl.expr.exthdr { name = "frag"; field = "nexthdr"; }) {
  ...
  testExprConcat = pe (dsl.expr.concat [
    dsl.fields.ip.saddr
  ...
  testExprSet = pe (dsl.expr.set [ 22 80 443 ]) { set = [ 22 80 443 ]; };

  testExprMap = pe (dsl.expr.map {
  ...
  testExprRange = pe (dsl.expr.range 1024 65535) { range = [ 1024 65535 ]; };

  testExprNumgen = pe (dsl.expr.numgen { mode = "inc"; mod = 4; offset = 100; }) {
    numgen = { mode = "inc"; mod = 4; offset = 100; };
  };

  testExprJhash = pe (dsl.expr.jhash {
    mod = 256;
  ...
  testExprSymhash = pe (dsl.expr.symhash { mod = 8; offset = 1; }) {
    symhash = { mod = 8; offset = 1; };
  };

  testExprFibRefined = pe (dsl.expr.fib { result = "oif"; flags = [ "saddr" "mark" ]; }) {
    fib = { result = "oif"; flags = [ "saddr" "mark" ]; };
  };

  testExprCtRefined = pe (dsl.expr.ct {
    key = "saddr";
    dir = "original";
    family = "ip";
  }) { ct = { key = "saddr"; dir = "original"; family = "ip"; }; };

  testExprRtRefined = pe (dsl.expr.rt { key = "nexthop"; family = "ip6"; }) {
    rt = { key = "nexthop"; family = "ip6"; };
  };

  testExprOsf = pe (dsl.expr.osf { key = "name"; ttl = "loose"; }) {
    osf = { key = "name"; ttl = "loose"; };
  };

  testExprIpsecRefined = pe (dsl.expr.ipsec {
    key = "saddr";
    family = "ip";
    dir = "in";
  }) { ipsec = { key = "saddr"; family = "ip"; dir = "in"; }; };

  testExprElem = pe (dsl.expr.elem { val = "10.0.0.1"; timeout = 60; }) {
  ...
  testExprBitxor = pe (dsl.expr.bitxor 1 2) { "^" = [ 1 2 ]; };
  testExprBitand = pe (dsl.expr.bitand dsl.fields.meta.mark 255) {
    "&" = [ { meta = { key = "mark"; }; } 255 ];
  };
  testExprLshift = pe (dsl.expr.lshift 1 4) { "<<" = [ 1 4 ]; };
  testExprRshift = pe (dsl.expr.rshift 16 4) { ">>" = [ 16 4 ]; };
  ...
  testVerdictJumpStmt = ps (dsl.jump "inbound") { jump = { target = "inbound"; }; };
  testVerdictGotoStmt = ps (dsl.goto "elsewhere") { goto = { target = "elsewhere"; }; };

  testVerdictAcceptExpr = pe dsl.accept { accept = null; };
  testVerdictJumpExpr = pe (dsl.jump "vmap-target") { jump = { target = "vmap-target"; }; };
  ...
  testOpNe = ps (dsl.ne 1 2) { match = { left = 1; right = 2; op = "!="; }; };
  testOpLt = ps (dsl.lt 1 2) { match = { left = 1; right = 2; op = "<"; }; };
  testOpGt = ps (dsl.gt 1 2) { match = { left = 1; right = 2; op = ">"; }; };
  testOpLe = ps (dsl.le 1 2) { match = { left = 1; right = 2; op = "<="; }; };
  testOpGe = ps (dsl.ge 1 2) { match = { left = 1; right = 2; op = ">="; }; };

  # inSet with list rhs auto-wraps as { set = ... }
  testOpInSetList = ps (dsl.inSet dsl.fields.ct.state [ "established" "related" ]) {
    match = {
      left = { ct = { key = "state"; }; };
  ...
  testOpNotInSetList = ps (dsl.notInSet dsl.fields.tcp.dport [ 22 80 ]) {
    match = {
      left = { payload = { protocol = "tcp"; field = "dport"; }; };
      right = { set = [ 22 80 ]; };
      op = "!=";
  ...
  # within synonym
  testOpWithin = ps (dsl.within dsl.fields.tcp.dport [ 1024 65535 ]) {
    match = {
      left = { payload = { protocol = "tcp"; field = "dport"; }; };
      right = { set = [ 1024 65535 ]; };
      op = "==";
    };
  };
  ...
  testOpMatchRaw = ps (dsl.match.raw {
    op = ">=";
  ...
  testCounterBase = ps (dsl.counter { packets = 0; bytes = 0; }) {
  ...
  testRejectBase = ps (dsl.reject { type = "icmp"; expr = "host-unreachable"; }) {
  ...
  testRejectTcpReset = ps dsl.reject.tcpReset { reject = { type = "tcp reset"; }; };

  # log — base, .plain, camelCase rename
  testLogBase = ps (dsl.log {
    prefix = "DROPPED: ";
    level = "info";
  ...
  testLogQueueThreshold = ps (dsl.log { queueThreshold = 50; }) {
    log = { "queue-threshold" = 50; };
  };

  # limit / quota
  testLimitBase = ps (dsl.limit { rate = 10; per = "minute"; burst = 5; }) {
    limit = { rate = 10; per = "minute"; burst = 5; };
  };
  testLimitRef = ps (dsl.limit.ref "lim") { limit = "lim"; };
  testQuotaBase = ps (dsl.quota { val = 1000; val_unit = "mbytes"; }) {
    quota = { val = 1000; val_unit = "mbytes"; };
  };
  testQuotaRef = ps (dsl.quota.ref "q1") { quota = "q1"; };

  # NAT family
  testSnat = ps (dsl.snat { addr = "192.0.2.1"; family = "ip"; }) {
    snat = { addr = "192.0.2.1"; family = "ip"; };
  };
  testDnat = ps (dsl.dnat { addr = "10.0.0.1"; family = "ip"; port = 8080; }) {
  ...
  testRedirectBase = ps (dsl.redirect { port = 8080; }) { redirect = { port = 8080; }; };
  testRedirectPlain = ps dsl.redirect.plain { redirect = { }; };
  testFwd = ps (dsl.fwd { dev = "eth0"; }) { fwd = { dev = "eth0"; }; };
  testDup = ps (dsl.dup { addr = "10.0.0.1"; dev = "eth0"; }) {
    dup = { addr = "10.0.0.1"; dev = "eth0"; };
  };
  testTproxy = ps (dsl.tproxy { family = "ip"; addr = "127.0.0.1"; port = 8080; }) {
    tproxy = { family = "ip"; addr = "127.0.0.1"; port = 8080; };
  };

  # synproxy
  testSynproxyBase = ps (dsl.synproxy { mss = 1460; wscale = 7; }) {
  ...
  # queue
  testQueueBase = ps (dsl.queue { num = 1; flags = [ "bypass" ]; }) {
  ...
  testCtCount = ps (dsl.ctCount { val = 100; inv = true; }) {
  ...
  testMeter = ps (dsl.meter {
    name = "m";
    key = dsl.fields.meta.iifname;
    stmt = dsl.counter { packets = 0; bytes = 0; };
    size = 4096;
  }) {
    meter = {
      name = "m";
  ...
  testSetStmt = ps (dsl.setStmt { op = "add"; elem = "10.0.0.1"; set = "@blocked"; }) {
    set = { op = "add"; elem = "10.0.0.1"; set = "@blocked"; };
  };
  testMapStmt = ps (dsl.mapStmt {
    op = "update";
    elem = "10.0.0.1";
    data = 42;
    map = "@cache";
  ...
  testXt = ps (dsl.xt "match" "connlimit") { xt = { type = "match"; name = "connlimit"; }; };
  testLast = ps dsl.last { last = null; };
  testLastUsed = ps (dsl.lastUsed (-1)) { last = { used = -1; }; };
  ...
  testRenderTableWithOptions = pr
    ...
    ])
    {
      nftables = [
        { flush = { ruleset = null; }; }
        { add = { table = { family = "inet"; name = "t"; comment = "demo"; }; }; }
      ];
    };

  # Set inside table — demonstrates `elements` (DSL) → `elem` (JSON) rename.
  testRenderTableWithSet = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        sets.trusted = {
          type = "ipv4_addr";
  ...
  testRenderSetElementsRename = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        sets.foo = {
          type = "ipv4_addr";
  ...
  testRenderElementObjectRename = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        elements.blocked = {
  ...
  testRenderChainWithRules = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        chains.input = {
          ...
          rules = [
            [ dsl.accept ]
            [ (dsl.eq dsl.fields.tcp.dport 22) dsl.accept ]
          ];
        };
      })
    ])
    {
      nftables = [
        ...
        { add = { rule = {
          family = "inet"; table = "t"; chain = "input";
          expr = [ { accept = null; } ];
        }; }; }
        { add = { rule = {
          family = "inet"; table = "t"; chain = "input";
  ...
  testRenderTwoChains = pr
    ...
    ])
    {
      nftables = [
        { add = { table = { family = "inet"; name = "t"; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "input"; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "output"; }; }; }
        { add = { rule = {
          family = "inet"; table = "t"; chain = "input";
          expr = [ { drop = null; } ];
        }; }; }
        { add = { rule = {
          family = "inet"; table = "t"; chain = "output";
          expr = [ { accept = null; } ];
        }; }; }
      ];
    };
  ...
  testRenderRuleAttrset = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        chains.c = {
          rules = [
            { expr = [ dsl.accept ]; handle = 42; comment = "the rule"; }
          ];
        };
      })
    ])
  ...
  testRenderMixRawAndTable = pr
    (dsl.ruleset [
      dsl.flush
      (dsl.table "ip" "x" { })
      { add = { table = { family = "ip"; name = "y"; }; }; }
    ])
    {
      nftables = [
        { flush = { ruleset = null; }; }
  ...
  testRenderAllObjectKinds = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        counters.hits = { packets = 0; bytes = 0; };
        quotas.q = { bytes = 1000; };
        limits.lim = { rate = 10; per = "second"; };
        secmarks.s = { context = "u:r:t:s0"; };
        synproxies.sp = { mss = 1460; wscale = 7; flags = [ "timestamp" ]; };
      })
    ])
    {
      nftables = [
        { add = { table = { family = "inet"; name = "t"; }; }; }
        { add = { counter = {
          family = "inet"; table = "t"; name = "hits";
          packets = 0; bytes = 0;
        }; }; }
        { add = { limit = {
          family = "inet"; table = "t"; name = "lim";
          rate = 10; per = "second";
        }; }; }
        { add = { quota = {
          family = "inet"; table = "t"; name = "q";
          bytes = 1000;
        }; }; }
        { add = { secmark = {
  ...
  testRenderNestedListFlatten = pr
    (dsl.ruleset [
      dsl.flush
      [
        (dsl.table "ip" "a" { })
    ...
    ])
    {
      nftables = [
        { flush = { ruleset = null; }; }
        { add = { table = { family = "ip"; name = "a"; }; }; }
        { add = { table = { family = "ip"; name = "b"; }; }; }
        { add = { table = { family = "ip"; name = "c"; }; }; }
      ];
    };

  # Deeply nested lists still flatten to a single-level command list.
  testRenderDeepNestedListFlatten = pr
    (dsl.ruleset [
      [ [ [ dsl.flush ] ] ]
      [ (dsl.table "ip" "x" { }) ]
    ])
  ...
  testRenderTableEmptyChains = pr
    (dsl.ruleset [ (dsl.table "inet" "t" { chains = { }; }) ])
    { nftables = [ { add = { table = { family = "inet"; name = "t"; }; }; } ]; };

  # Chain with an empty `rules` list emits only the chain-add command.
  testRenderChainEmptyRules = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        chains.c = { rules = [ ]; };
      })
  ...
  testRenderMultiTable = pr
    (dsl.ruleset [
      dsl.flush
      (dsl.table "inet" "filter" {
        chains.input = {
          type = "filter"; hook = "input"; prio = 0; policy = "drop";
          rules = [ [ (dsl.inSet dsl.fields.ct.state [ "established" ]) dsl.accept ] ];
        };
      })
      (dsl.table "ip" "nat" {
        chains.postrouting = {
  ...
  testRenderNetdevIngress = pr
    (dsl.ruleset [
      (dsl.table "netdev" "guard" {
        chains.ingress = {
          type = "filter"; hook = "ingress"; prio = -500;
          dev = [ "eth0" "eth1" ];
          policy = "drop";
          rules = [
            [ (dsl.eq dsl.fields.ip.saddr "@blocklist") dsl.drop ]
          ];
        };
      })
    ])
    {
      nftables = [
        { add = { table = { family = "netdev"; name = "guard"; }; }; }
        { add = { chain = {
          family = "netdev"; table = "guard"; name = "ingress";
          type = "filter"; hook = "ingress"; prio = -500;
          dev = [ "eth0" "eth1" ]; policy = "drop";
        }; }; }
        { add = { rule = {
          family = "netdev"; table = "guard"; chain = "ingress";
          expr = [
            { match = {
              left = { payload = { protocol = "ip"; field = "saddr"; }; };
              right = "@blocklist"; op = "==";
  ...
  testRenderVmapDispatch = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        ...
        chains.wan = { rules = [ [ dsl.accept ] ]; };
        chains.lan = { rules = [ [ dsl.accept ] ]; };
        chains.input = {
          type = "filter"; hook = "input"; prio = 0; policy = "drop";
    ...
    ])
    {
      nftables = [
        ...
        { add = { chain = { family = "inet"; table = "t"; name = "lan"; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "wan"; }; }; }
        # Object-adds next — `wan`/`lan` already exist so verdict elements resolve.
        { add = { map = {
          family = "inet"; table = "t"; name = "iif_dispatch";
          type = "ifname"; map = "verdict";
          elem = [
            [ "eth0" { jump = { target = "wan"; }; } ]
            [ "eth1" { jump = { target = "lan"; }; } ]
          ];
        }; }; }
        ...
        { add = { rule = {
          family = "inet"; table = "t"; chain = "lan";
          expr = [ { accept = null; } ];
        }; }; }
        { add = { rule = {
          family = "inet"; table = "t"; chain = "wan";
          expr = [ { accept = null; } ];
        }; }; }
      ];
    };
  ...
  testRenderFlowOffload = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        flowtables.ft = {
          hook = "ingress"; prio = 0; dev = [ "eth0" "eth1" ];
        };
        chains.forward = {
          type = "filter"; hook = "forward"; prio = 0;
          rules = [
            [
              (dsl.inSet dsl.fields.ct.state [ "established" "related" ])
    ...
    ])
    {
      nftables = [
        { add = { table = { family = "inet"; name = "t"; }; }; }
        { add = { chain = {
          family = "inet"; table = "t"; name = "forward";
          type = "filter"; hook = "forward"; prio = 0;
        }; }; }
        { add = { flowtable = {
        ...
        { add = { rule = {
          family = "inet"; table = "t"; chain = "forward";
          expr = [
            { match = {
              left = { ct = { key = "state"; }; };
              right = { set = [ "established" "related" ]; };
  ...
  testRenderConcatenatedPortForward = pr
    (dsl.ruleset [
      (dsl.table "ip" "nat" {
        maps.port_forward = {
          ...
          flags = [ "interval" ];
          elements = [
            [ (dsl.expr.concat [ "203.0.113.1" 80 ])  (dsl.expr.concat [ "10.0.0.10" 8080 ]) ]
            [ (dsl.expr.concat [ "203.0.113.1" 443 ]) (dsl.expr.concat [ "10.0.0.10" 8443 ]) ]
          ];
        };
        chains.prerouting = {
          ...
          rules = [
            [
              (dsl.dnat {
                family = "ip";
                addr = dsl.expr.map {
                  key = dsl.expr.concat [ dsl.fields.ip.daddr dsl.fields.tcp.dport ];
                  data = "@port_forward";
                };
              })
            ]
    ...
    ])
    {
      nftables = [
        ...
        { add = { map = {
          ...
          flags = [ "interval" ];
          elem = [
            [ { concat = [ "203.0.113.1" 80 ]; }  { concat = [ "10.0.0.10" 8080 ]; } ]
            [ { concat = [ "203.0.113.1" 443 ]; } { concat = [ "10.0.0.10" 8443 ]; } ]
          ];
        }; }; }
        { add = { rule = {
  ...
  testRenderInlineSetMatch = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        chains.input = {
          rules = [
            [ (dsl.inSet dsl.fields.tcp.dport [ 22 80 443 ]) dsl.accept ]
          ];
        };
      })
    ])
  ...
  testRenderAllInetHooks = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        chains.prerouting = { type = "filter"; hook = "prerouting"; prio = 0; rules = [ [ dsl.accept ] ]; };
        chains.input = { type = "filter"; hook = "input"; prio = 0; rules = [ [ dsl.accept ] ]; };
        chains.forward = { type = "filter"; hook = "forward"; prio = 0; rules = [ [ dsl.accept ] ]; };
        chains.output = { type = "filter"; hook = "output"; prio = 0; rules = [ [ dsl.accept ] ]; };
        chains.postrouting = { type = "filter"; hook = "postrouting"; prio = 0; rules = [ [ dsl.accept ] ]; };
      })
    ])
    {
      ...
      nftables = [
        { add = { table = { family = "inet"; name = "t"; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "forward";
          type = "filter"; hook = "forward"; prio = 0; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "input";
          type = "filter"; hook = "input"; prio = 0; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "output";
          type = "filter"; hook = "output"; prio = 0; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "postrouting";
          type = "filter"; hook = "postrouting"; prio = 0; }; }; }
        { add = { chain = { family = "inet"; table = "t"; name = "prerouting";
          type = "filter"; hook = "prerouting"; prio = 0; }; }; }
        { add = { rule = { family = "inet"; table = "t"; chain = "forward";
          expr = [ { accept = null; } ]; }; }; }
        { add = { rule = { family = "inet"; table = "t"; chain = "input";
          expr = [ { accept = null; } ]; }; }; }
        { add = { rule = { family = "inet"; table = "t"; chain = "output";
          expr = [ { accept = null; } ]; }; }; }
        { add = { rule = { family = "inet"; table = "t"; chain = "postrouting";
          expr = [ { accept = null; } ]; }; }; }
        { add = { rule = { family = "inet"; table = "t"; chain = "prerouting";
          expr = [ { accept = null; } ]; }; }; }
      ];
    };

  # Realistic multi-statement rule: log + rate-limit + reject with icmpx code.
  testRenderLogRejectRule = pr
    (dsl.ruleset [
      (dsl.table "inet" "t" {
        chains.input = {
          type = "filter"; hook = "input"; prio = 0; policy = "drop";
          rules = [
            [
              (dsl.log { prefix = "DROP: "; level = "info"; })
              (dsl.limit { rate = 5; per = "second"; burst = 10; })
      ...
      nftables = [
        ...
        { add = { rule = {
          ...
          expr = [
            ...
            { limit = { rate = 5; per = "second"; burst = 10; }; }
            { reject = { type = "icmpx"; expr = "admin-prohibited"; }; }
          ];
        }; }; }
      ];
  ...
  testDslExampleValidates = {
    expr = builtins.isString (roundtrip nftlib.ruleset (
      import ../examples/basic-firewall-dsl.nix { inherit nftlib; }
    ));
    expected = true;
  };
  ...
  testDslHomeRouterValidates = {
    expr = builtins.isString (roundtrip nftlib.ruleset (
      import ../examples/home-router-dsl.nix { inherit nftlib; }
    ));
    expected = true;
  };
  ...
  testCmdCreateTable = pc (dsl.create.table { family = "ip"; name = "t"; }) {
    create = { table = { family = "ip"; name = "t"; }; };
  };
  testCmdCreateCounter = pc
    (dsl.create.counter { family = "ip"; table = "t"; name = "c"; })
    { create = { counter = { family = "ip"; table = "t"; name = "c"; }; }; };
  testCmdCreateCtHelper = pc
    (dsl.create.ctHelper { family = "ip"; table = "t"; name = "h"; type = "ftp"; protocol = "tcp"; })
    { create = { "ct helper" = {
        family = "ip"; table = "t"; name = "h"; type = "ftp"; protocol = "tcp";
      }; }; };
  ...
  testCmdDeleteSet = pc
    ...
    { delete = { set = {
        family = "inet"; table = "t"; name = "blocked"; type = "ipv4_addr";
      }; }; };
  testCmdDeleteChain = pc
    (dsl.delete.chain { family = "ip"; table = "t"; name = "c"; })
    { delete = { chain = { family = "ip"; table = "t"; name = "c"; }; }; };

  testCmdDestroyTable = pc
    (dsl.destroy.table { family = "ip"; name = "old"; })
    { destroy = { table = { family = "ip"; name = "old"; }; }; };
  testCmdDestroyQuota = pc
    (dsl.destroy.quota { family = "ip"; table = "t"; name = "q"; })
    { destroy = { quota = { family = "ip"; table = "t"; name = "q"; }; }; };

  testCmdListTable = pc (dsl.list.table { family = "inet"; name = "filter"; }) {
    list = { table = { family = "inet"; name = "filter"; }; };
  };
  testCmdListMetainfo = pc
    (dsl.list.metainfo { version = "1.1.6"; release_name = "Commodore Bullmoose"; json_schema_version = 1; })
    { list = { metainfo = {
        version = "1.1.6"; release_name = "Commodore Bullmoose"; json_schema_version = 1;
      }; }; };
  ...
  testCmdRenameChain = pc
    (dsl.rename.chain { family = "ip"; table = "t"; name = "old"; newname = "new"; })
    { rename = { chain = { family = "ip"; table = "t"; name = "old"; newname = "new"; }; }; };
  ...
  testCmdResetCounter = pc
    (dsl.reset.counter { family = "ip"; table = "t"; name = "c"; })
    { reset = { counter = { family = "ip"; table = "t"; name = "c"; }; }; };
  testCmdResetRule = pc
    (dsl.reset.rule { family = "ip"; table = "t"; chain = "c"; handle = 42; expr = [ { accept = null; } ]; })
    { reset = { rule = {
        family = "ip"; table = "t"; chain = "c"; handle = 42;
        expr = [ { accept = null; } ];
      }; }; };
  ...
  testCmdReplace = pc
    (dsl.replace { family = "ip"; table = "t"; chain = "c"; handle = 42; expr = [ { accept = null; } ]; })
    { replace = { rule = {
        family = "ip"; table = "t"; chain = "c"; handle = 42;
        expr = [ { accept = null; } ];
      }; }; };
  testCmdInsert = pc
    (dsl.insert { family = "ip"; table = "t"; chain = "c"; index = 0; expr = [ { drop = null; } ]; })
    { insert = { rule = {
        family = "ip"; table = "t"; chain = "c"; index = 0;
        expr = [ { drop = null; } ];
      }; }; };
  ...
  testCmdFlushBare = pc dsl.flush { flush = { ruleset = null; }; };
  testCmdFlushRuleset = pc (dsl.flushRuleset { family = "ip"; }) {
    flush = { ruleset = { family = "ip"; }; };
  };
  testCmdFlushTable = pc (dsl.flushTable { family = "inet"; name = "filter"; }) {
    flush = { table = { family = "inet"; name = "filter"; }; };
  };
  testCmdFlushChain = pc (dsl.flushChain { family = "inet"; table = "t"; name = "input"; }) {
    flush = { chain = { family = "inet"; table = "t"; name = "input"; }; };
  };
  testCmdFlushSet = pc
    (dsl.flushSet { family = "inet"; table = "t"; name = "trusted"; type = "ipv4_addr"; })
    { flush = { set = {
        family = "inet"; table = "t"; name = "trusted"; type = "ipv4_addr";
      }; }; };
  testCmdFlushMap = pc
    (dsl.flushMap {
      family = "ip"; table = "nat"; name = "port_forward";
  ...
  testCmdRulesetIntegration = pr
    (dsl.ruleset [
      dsl.flush
      (dsl.create.table { family = "ip"; name = "t"; })
      (dsl.rename.chain {
        family = "ip"; table = "t"; name = "oldchain"; newname = "newchain";
      })
      (dsl.replace {
        family = "ip"; table = "t"; chain = "c"; handle = 7;
        expr = [ { accept = null; } ];
      })
      (dsl.reset.counter { family = "ip"; table = "t"; name = "c"; })
      (dsl.destroy.set { family = "ip"; table = "t"; name = "stale"; type = "ipv4_addr"; })
    ])
    {
      nftables = [
        { flush = { ruleset = null; }; }
        ...
        { reset = { counter = { family = "ip"; table = "t"; name = "c"; }; }; }
        { destroy = { set = { family = "ip"; table = "t"; name = "stale"; type = "ipv4_addr"; }; }; }
      ];
    };
}
