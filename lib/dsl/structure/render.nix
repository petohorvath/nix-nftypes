...
let
  ...
  objectKinds = {
    sets = { tag = "set"; renameBody = rename.set; };
    maps = { tag = "map"; renameBody = rename.set; };
    elements = { tag = "element"; renameBody = rename.element; };
    flowtables = { tag = "flowtable"; renameBody = lib.id; };
    counters = { tag = "counter"; renameBody = lib.id; };
    quotas = { tag = "quota"; renameBody = lib.id; };
    limits = { tag = "limit"; renameBody = lib.id; };
    ctHelpers = { tag = "ct helper"; renameBody = lib.id; };
    ctTimeouts = { tag = "ct timeout"; renameBody = lib.id; };
    ctExpectations = { tag = "ct expectation"; renameBody = lib.id; };
    secmarks = { tag = "secmark"; renameBody = lib.id; };
    synproxies = { tag = "synproxy"; renameBody = lib.id; };
    ...
    tunnels = { tag = "tunnel"; renameBody = rename.tunnel; };
  };
  ...
  emitObject = ctx: pluralKey: name: body:
    let
      cfg = objectKinds.${pluralKey};
      full = compact (
        { inherit (ctx) family table; inherit name; } // (cfg.renameBody body)
      );
    in
    { add = { "${cfg.tag}" = full; }; };

  # All `add <kind>` commands for every object kind present in `body`.
  emitObjects = ctx: body:
    let
      presentKinds = builtins.filter (k: body ? ${k}) (sortedNames objectKinds);
      emitKind = pluralKey:
        let entries = body.${pluralKey};
        in map (name: emitObject ctx pluralKey name entries.${name}) (sortedNames entries);
    in
  ...
  emitRule = ctx: entry:
    let
      ...
      full = compact (
        { inherit (ctx) family table chain; } // body
      );
    in
    { add = { rule = full; }; };

  # Emit `add chain` for a single chain (rules excluded — see emitChainRules).
  emitChainAdd = ctx: name: chainBody:
    let
      full = compact (
        { inherit (ctx) family table; inherit name; } // (removeAttrs chainBody [ "rules" ])
      );
    in
    { add = { chain = full; }; };

  # Emit all `add rule` commands for one chain.
  emitChainRules = ctx: name: chainBody:
  ...
  expandTable = node:
    let
      ...
      body = removeAttrs node [ markers.table "family" "name" ];
      tableOpts = builtins.intersectAttrs {
        handle = null;
        flags = null;
        comment = null;
      } body;
      tableAdd = { add = { table = compact ({ inherit family name; } // tableOpts); }; };
