...
- 81 round-trip tests wired into `nix flake check`.
...
```nix
{
  ...
  outputs = { self, nixpkgs, nix-nft-types }: {
    packages.x86_64-linux.rules =
      ...
      in
      nixpkgs.legacyPackages.x86_64-linux.writeText
        "rules.json" (nftlib.toJSON ruleset);
  };
}
```
...
See `examples/basic-firewall.nix` for a realistic inet-family ruleset with NAT, named sets, map-based port forwarding, and a vmap.
...
tests/default.nix    81 roundtrip tests
examples/basic-firewall.nix    realistic inet-family firewall
```
...
Rendering the example ruleset to JSON for inspection:

```
nix eval --impure --raw --expr '
  let
    flake = builtins.getFlake (toString ./.);
    example = import ./examples/basic-firewall.nix { nftlib = flake.lib; };
